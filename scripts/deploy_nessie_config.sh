...
local_config="/var/app/current/app/config/production-local.py"
...
env_name=$(echo "${eb_env}" | sed -E -e 's/(nessie-lowlands-|nessie-highlands-)//')
config_location="s3://la-deploy-configs/nessie/nessie-al2-${env_name}.py"
...
echo; echo "Done!"; echo
echo "Restart Nessie to pick up new configs. Have a nice day!"; echo
